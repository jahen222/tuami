...
public function airport()
{
    ...
    public function map_index(Request $request)
    {
		...
		$ip_detail = json_decode(@file_get_contents("http://www.geoplugin.net/json.gp?ip={$ip}"));
		
		if( $ip_detail ) {
			$latlng['lat'] = $ip_detail->geoplugin_latitude;
			$latlng['lng'] = $ip_detail->geoplugin_longitude;
		}
		
        return view('admin.map.index', compact('latlng'));
    }
    ...
    public function settings_payment_store(Request $request)
    {
        ...
        Setting::set('driver_base_price', $request->driver_base_price);
        Setting::set('fare_price', $request->fare_price);
