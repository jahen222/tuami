...
class UserRequests extends Model
{
    ...
    protected $fillable = [
        ...
        'signature',
        'total_time',
        'provider_service_type','schedule_end_date_time','service_hour','service_hour_status','selling_item_id','total_item','courier_id',
    ];
	...
	/**
     * The user who created the request selected column.
     */
    public function userSelectedColumn()
    {
        return $this->belongsTo('App\User','user_id')->select('id', 'first_name', 'payment_mode','email');
    }
    ...
    {
        return $this->hasMany('App\ItemRequest', 'user_id', 'user_id')->where('status', '=', '1')->leftjoin('selling_items', 'item_requests.selling_item_id', 'selling_items.id'); 
    }
	
	
	
	/**
     * The provider assigned to the request selected column.
     */
    public function providerSelectedColumn() 
    {
        return $this->belongsTo('App\Provider', 'provider_id')->select('id', 'first_name', 'mobile', 'email', 'rating');
    }
	
	public function storeUser(){
		return $this->belongsTo('App\StoreUser', 'service_type_id');
	}
	
	public function storeUserSelectedColumn(){ 
		return $this->belongsTo('App\StoreUser', 'service_type_id')->select('id', 'name', 'email', 'store_type', 'store_name', 'image', 'banner_image', 'min_amount', 'offer_in_percentage', 'address','landmark');
	}
	...
	public function store_payment($query)
    {
        return $this->belongsTo('App\ProviderService', 'provider_id', 'provider_id');
    }
    ...
    public function scopeUserTrips($query, $user_id)
    {
     ...
     public function scopeCurrentUserTrips($query, $user_id,$check_status)
    {
        return $query->where('user_requests.user_id', '=', $user_id)
